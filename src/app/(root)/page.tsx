...
export default function SiteListPage() {
	...
	// Only connect once the bulk list from listSites() has landed, so events are always applied
	// on top of it instead of racing it (which trickled sites in one by one). `watchKubernetesSites`
	// starts each watch from its own connection-time resourceVersion, so connecting slightly later
	// here doesn't miss anything.
	useServerEvents<{ site: SiteEvent }>(
		"/api/sites/events",
		{
			site: (event) => {
				switch (event.type) {
	...
	const blankFilters: SiteListFilters = {
		url: "",
		infrastructure: "",
		theme: "",
		hasCategories: null,
	...
	const [filters, setFilters] = useState<SiteListFilters>(getInitialFilters());
	...
	const updateURL = (filters: SiteListFilters) => {
		...
		if (filters.hasCategories !== null) params.set("hasCategories", filters.hasCategories.toString());
		if (filters.hasDownloadsProtection !== null) params.set("hasDownloadsProtection", filters.hasDownloadsProtection.toString());
		if (filters.dateRange.from) params.set("dateFrom", filters.dateRange.from.toISOString().split("T")[0]);
		if (filters.dateRange.to) params.set("dateTo", filters.dateRange.to.toISOString().split("T")[0]);
	...
	const updateFilters = (newFilters: SiteListFilters) => {
	...
	const getInfrastructureConfig = (typeName: string) => {
	...
	const filteredSites = sites.filter((site) => {
		...
		const matchesDownloadsProtection =
			filters.hasDownloadsProtection === null || (isKubernetesSite(site) && site.downloadsProtectionScript !== undefined && site.downloadsProtectionScript === filters.hasDownloadsProtection);
		...
		},
		{
			...
			render: (site) => (
				<div className="text-center">
					<p className="text-sm font-medium text-gray-700" title={site.createdAt.toString()}>
							...
							<DialogContent className="max-w-md">
								...
								<div className="space-y-1">
									{toolsMenuItems.map((tool) => (
										<Button key={tool.id} variant="ghost" className="w-full justify-start h-auto p-4 text-left" onClick={() => handleToolSelect(tool.id)}>
													...
													<div key={language.locale} className="flex items-center space-x-2">
														<Checkbox
															...
															onCheckedChange={(checked) => {
																if (checked) updateFilters({ ...filters, languages: [...filters.languages, language.locale] });
																else updateFilters({ ...filters, languages: filters.languages.filter((l) => l !== language.locale) });
															}}
														/>
														<label htmlFor={`lang-${language.locale}`} className="text-sm">
															{language.common}
														</label>
													</div>
												))}
												...
												{Object.values(OPTIONAL_CATEGORIES).map((category) => (
													<div key={category.NAME} className="flex items-center space-x-2">
														<Checkbox
															...
															onCheckedChange={(checked) => {
																if (checked) updateFilters({ ...filters, categories: [...filters.categories, category.NAME] });
																else updateFilters({ ...filters, categories: filters.categories.filter((c) => c !== category.NAME) });
															}}
														/>
														<label htmlFor={`cat-${category.NAME}`} className="text-sm">
															{category.LABEL}
														</label>
													</div>
												))}
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<div>
											...
											<Input
												...
												onChange={(e) => updateFilters({ ...filters, dateRange: { ...filters.dateRange, from: e.target.value ? new Date(e.target.value) : undefined } })}
											/>
										</div>
										<div>
											...
											<Input
												...
												onChange={(e) => updateFilters({ ...filters, dateRange: { ...filters.dateRange, to: e.target.value ? new Date(e.target.value) : undefined } })}
											/>
										</div>
									</div>
												...
												<div className="space-y-3">
													<div className="flex justify-between items-center">
														...
														<div className="flex items-center gap-2">
															...
															<span className="text-xs text-gray-600 w-10 text-right font-medium">{Math.round((statistics.kubernetesSites / statistics.totalSites) * 100)}%</span>
														</div>
													</div>
													<div className="flex justify-between items-center">
														...
														<div className="flex items-center gap-2">
															...
															<span className="text-xs text-gray-600 w-10 text-right font-medium">{Math.round((statistics.databaseSites / statistics.totalSites) * 100)}%</span>
														</div>
													</div>
													<div className="flex justify-between items-center">
														...
														<div className="flex items-center gap-2">
															...
															<span className="text-xs text-gray-600 w-10 text-right font-medium">{Math.round((statistics.noneSites / statistics.totalSites) * 100)}%</span>
														</div>
													</div>
												</div>
											</div>
